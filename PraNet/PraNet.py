...
class PraNet(nn.Module):
    ...
    def __init__(self, channel=32):
        ...
        self.ra2_conv1 = BasicConv2d(512, 64, kernel_size=1)#outchannel=inchannel/F.interpolate中的scale_factor
    ...
    def forward(self, x):
        ...
        ra5_feat = self.agg1(x4_rfb, x3_rfb, x2_rfb)     # 大小为 1 44 44
        ...
        #把PD后的结果（1*44*44）缩放成和f5（2048*11*11）一样的大小
        crop_4 = F.interpolate(ra5_feat, scale_factor=0.25, mode='bilinear')
        # 反转元素
        x = -1*(torch.sigmoid(crop_4)) + 1
        #对改变尺寸后的Global Map的channel进行维度扩张，扩张成和f5同通道（2048）同尺寸，然后和该维度特征图f5进行乘积
        x = x.expand(-1, 2048, -1, -1).mul(x4)#扩展张量中某维数据的尺寸，扩展维度会复制原有的数据进行扩展
        #对数据进行降channel
        x = self.ra4_conv1(x)
        ...
        #将R5和调整过大小的PD结果相加生成S5
        x = ra4_feat + crop_4
        #将S5的尺寸改到和输入图同尺寸作为一个深监督
